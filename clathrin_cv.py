...
for train, test in kfold.split(X, Y):
    model = cnn_model()   
    ## evaluate the model
    model.fit(X[train], np_utils.to_categorical(Y[train],nb_classes), epochs=150, batch_size=10, verbose=0)
    # evaluate the model
...
## balance data
model.fit(X, np_utils.to_categorical(Y, nb_classes), epochs=150, batch_size=10, verbose=0, class_weight='auto')
### evaluate the model
